...
export default function ServiceDetailClient({ service }: { service: IService | null }) {
  ...
  useEffect(() => {
    // lấy danh sách khác (loại trừ bài hiện tại) cho mục "dịch vụ liên quan"
    const fetchOthers = async () => {
      try {
        const res = await fetch('/api/services');
        const data = await res.json();
        if (data.success && data.data) {
          setAllServices((data.data as IService[]).filter((x) => !x.link?.endsWith(`/${slug}`)));
        }
      } catch {
        /* không chặn trang */
      } finally {
