...
export default function CreateTestimonialPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success && data.data) {
        toast.success(data.message || 'Tạo lời chứng thực thành công!');
        router.push('/admin/testimonials');
      } else {
        // Handle field-specific errors from backend
        if (data.statusCode === 400 && data.message) {
          if (data.message.includes('Name (en, vi), image, and content')) {
            setErrors({
  ...
  return (
    <div className="container mx-auto">
      <h1 className="text-2xl font-bold mb-4">Tạo Lời Chứng Thực Mới</h1>
