  ...
  return (
    <motion.div className="space-y-6" variants={v} initial="hidden" animate="visible">
      ...
      <motion.div variants={iv} className="relative max-w-md">
        ...
        <Input placeholder="Tìm kiếm theo tên hoặc chức vụ..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 h-11 rounded-xl border-slate-200 focus:border-blue-400" />
      </motion.div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredMembers.map((member) => (
                      <TableRow key={member._id?.toString()} className="hover:bg-blue-50/30 transition-colors">
                        <TableCell>
