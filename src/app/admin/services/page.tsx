...
import { Pencil, Trash2, Search, Plus, Briefcase, RefreshCw } from 'lucide-react';
...
export default function ServicesPage() {
  ...
  const fetchServices = async () => {
    ...
    try {
      ...
      else { toast.error(data.message || 'Không thể tải dịch vụ'); setError(data.message || 'Không thể tải dịch vụ'); }
    } catch (error: unknown) {
  ...
  const filteredServices = useMemo(() => {
    return services.filter((s) =>
      s.name.vi.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
      s.name.en.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
      (s.link || '').toLowerCase().includes(debouncedSearchTerm.toLowerCase())
    );
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (data.success) { toast.success(data.message || 'Đã xóa dịch vụ'); fetchServices(); }
          ...
          <CardContent className="p-0">
            {loading ? (
              ...
              <div className="p-12 text-center">
                ...
                <Button variant="outline" className="mt-4 rounded-xl" onClick={fetchServices}>Thử lại</Button>
              </div>
            ) : filteredServices.length === 0 ? (
              <div className="p-12 text-center">
                <div className="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center mx-auto mb-3"><Briefcase className="w-5 h-5 text-slate-400" /></div>
                <p className="text-slate-500 font-medium">Không có dịch vụ nào</p>
                <p className="text-sm text-slate-400 mt-1">Tạo dịch vụ mới để bắt đầu</p>
              </div>
            ) : (
                  ...
                  <TableBody>
                    {filteredServices.map((service) => (
                      <TableRow key={service._id?.toString()} className="transition-colors">
                        ...
                        <TableCell>
                          {service.link ? <a href={service.link} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline text-sm">{service.link}</a> : <span className="text-slate-400">—</span>}
                        </TableCell>
                        <TableCell className="text-sm text-slate-500">
                          {service.createdAt ? new Date(service.createdAt).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '—'}
                        </TableCell>
                        <TableCell>
                          <div className="flex justify-center gap-1.5">
                            ...
                            <AlertDialog open={deleteId === service._id} onOpenChange={(open) => !open && setDeleteId(null)}>
                              <AlertDialogTrigger asChild>
                                <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg" onClick={() => setDeleteId(service._id?.toString() || '')} disabled={deletingId === service._id}><Trash2 className="w-3.5 h-3.5" /></Button>
                              </AlertDialogTrigger>
                              <AlertDialogContent className="rounded-2xl">
                                <AlertDialogHeader><AlertDialogTitle>Xóa dịch vụ này?</AlertDialogTitle><AlertDialogDescription>Hành động này không thể hoàn tác.</AlertDialogDescription></AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel className="rounded-xl" disabled={deletingId === service._id}>Hủy</AlertDialogCancel>
                                  <AlertDialogAction onClick={() => handleDelete(service._id?.toString() || '')} className="bg-red-600 hover:bg-red-700 rounded-xl" disabled={deletingId === service._id}>{deletingId === service._id ? 'Đang xóa...' : 'Xóa'}</AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                          </div>
